...
public class LFUCache<K extends Comparable<K>, V> {
    ...
    public void set(K key, V value) {
        ...
        if (setElement != null) {
            // 如果之前已经加入过该对象, 先获取它所在的bucket
            Bucket<K, V> previousBucket = bucketMap.get(setElement.operations);
            // 从桶中移除该元素
            previousBucket.remove(setElement);
            // 该元素的操作次数+1
            setElement.operations++;
            // 判断操作次数+1的桶存不存在
            Bucket<K, V> nextBucket = bucketMap.get(setElement.operations);
            if (nextBucket != null) {
                // 如果存在, 则将该元素加到桶最后去
                nextBucket.addLast(setElement);
            } else {
                // 如果该桶不存在, 则新建
                nextBucket = new Bucket<>(setElement.operations);
                nextBucket.addLast(setElement);
                // 插入新桶, 保证升序
                ...
                // bucketMap记录
                bucketMap.put(setElement.operations, nextBucket);
            }
