...
public class MaximumSearchSubtree {
    ...
    public static class Info {
        ...
        public Info(Tree.Node<Integer> maximumSearchSubtreeRoot,
                    ...
                    Integer maxElement, Integer minElement,
    ...
    private static Info searchProcess(Tree.Node<Integer> curNode) {
        ...
        boolean curIsSearchTree = leftInfo.isSearchTree && rightInfo.isSearchTree
                && (leftInfo.maxElement == null || curNode.element > leftInfo.maxElement)
                && (rightInfo.minElement == null || curNode.element < rightInfo.minElement);
        ...
        Integer maxElement = curIsSearchTree ? rightInfo.maxElement == null ? curNode.element : rightInfo.maxElement
                // 如果不是搜索树, 如果左子树的最大值为空, 如果右子树的最大值为空, 则当前节点的element为最大值
                : leftInfo.maxElement == null ? rightInfo.maxElement == null ? curNode.element
                // 如果右子树最大值不为空, 则最大值应该比较当前节点和右子树的最大值
                : Math.max(curNode.element, rightInfo.maxElement)
                // 如果左子树最大值不为空, 如果右子树的最大值为空, 则最大值取当前节点和左子树最大值之一, 否则取3者最大值
                : Math.max(curNode.element, rightInfo.maxElement == null ? leftInfo.maxElement
                : Math.max(leftInfo.maxElement, rightInfo.maxElement));

        // 逻辑同取最大值
        Integer minElement = curIsSearchTree ? leftInfo.minElement == null ? curNode.element : leftInfo.minElement
                : leftInfo.minElement == null ? rightInfo.minElement == null ? curNode.element
                : Math.min(curNode.element, rightInfo.minElement)
                : Math.max(curNode.element, rightInfo.minElement == null ? leftInfo.minElement
                : Math.min(leftInfo.minElement, rightInfo.minElement));
